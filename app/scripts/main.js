...
var CLIENT_ID = '605645049061-a0oej37pp7nipl654qmmk75k6s5pb46l.apps.googleusercontent.com';
...
function queryBounceQuery(val, id, index) {
    'use strict';
    // Query the Core Reporting API for the number sessions for
    // the past seven days.
    gapi.client.analytics.data.ga.get({
        ...
        .then(function(response) {
            ...
            if(u === 0){
              ...
              if( isFinite(cr) || isFinite(u) || !isNaN(cr) || !isNaN(u) ){
                $(s).html(numeral(calculateMDU(u, cr, true)).format('0%'));
              } else {
                $(s).html('∞');
              }
